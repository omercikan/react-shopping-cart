...
export const ProductItem: React.FC<ProductItemProps> = ({item}) => {
  ...
  const findLikes = state.likes.find((product) => product.id === id);

  const handleLikes = () => {
    if(!findLikes) {
      dispatch({
        type: cartActionTypeEnum.ADD_LIKES,
        payload: item
      })
    } else {
      dispatch({
        type: cartActionTypeEnum.REMOVE_LIKES,
        payload: id
      })
    }
  }
    ...
    <li className='col-12 col-lg-3 col-md-4 col-sm-6 position-relative product-item'>
      <div className="card bg-secondary">
        <div className="product-like-item position-absolute top-25 end-50">
          <button className='bg-white border rounded-circle p-2 heart-btn' onClick={handleLikes}><i className={!findLikes ? "bi bi-heart" : 'bi bi-heart-fill active'}></i></button>
        </div>
